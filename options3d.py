...
def generate_plots(ticker, options, moneyness, params, price_type, current_date, current_time, *, scale=1.5, web=False):
    # Added scale and web keyword arguments
    # Set chart layout
    layout = [2, 4]
    rows = layout[0]
    cols = layout[1]
    ...
    # Change for web viewing

    if web:
        return jp.Matplotlib()
...
def create_inputs(wp):
    input_div = jp.Div(classes='flex m-2 p-2', a=wp)
    style = 'width: 150px'
    wp.ticker = jp.QInput(label='Enter ticker', hint='', a=input_div, classes='m-2', style='width: 100px', value='AAPL', outlined=True)
    wp.param_type = jp.QSelect(label='Param type', hint='', options=['Standard', 'Nonstandard'], a=input_div, classes='m-2', style=style, value='Standard', outlined=True)
    wp.price_type = jp.QSelect(label='Price type', hint='Select price to use for calculation', options=['mid', 'last'], a=input_div, classes='m-2', style=style, value='mid', outlined=True)
    wp.opt_type = jp.QSelect(label='Option type', hint='Select option type', options=['calls', 'puts', 'both'], a=input_div, classes='m-2', style=style, value='both', outlined=True)
    wp.moneyness = jp.QSelect(label='Moneyness', hint='Select moneyness to plot', options=['itm', 'otm', 'both'], a=input_div, classes='m-2', style=style, value='both', outlined=True)
    wp.r = jp.QInput(label='Enter risk free rate', hint='',a=input_div, classes='m-2', style='width: 150px', value=0.004, type='number', outlined=True)

    now = dt.datetime.now()
    wp.current_date = str(now.date())
    minute = now.time().minute
    wp.current_time = '{}:{}'.format(now.time().hour, minute)
    if minute < 10:
        wp.current_time = '{}:0{}'.format(now.time().hour, minute)
    ...
    wp.plot_button = jp.QBtn(label='Generate Plots', color='primary', a=input_div, classes='m-2 h-1')
    wp.input_div = input_div
    return wp

async def plot_charts(self, msg):
    wp = msg.page
    self.loading = True
    await msg.page.update()
    options = get_options(wp.current_date, wp.current_time, wp.ticker.value, wp.opt_type.value, wp.price_type.value, float(wp.r.value))
    if wp.param_type.value == 'Standard':
        params = ['V', 'IV', 'delta', 'theta', 'volume', 'vega', 'gamma', 'openInterest']
    elif wp.param_type.value == 'Nonstandard':
        print('\n**Warning: accuracy of higher order greeks has not been verified**\n')
        params = ['rho', 'charm', 'veta', 'color', 'speed', 'vanna', 'vomma', 'zomma']
    self.loading = False
    plots = generate_plots(wp.ticker.value, options, wp.moneyness.value, params, wp.price_type.value,
                           wp.date_time.value[0:11], wp.date_time.value[12:], scale=1.5, web=True)
    plots.classes = 'border m-2 p-2'
    wp.plots_div.add(plots)


def option_3d():
    wp = jp.QuasarPage(tailwind=True)
    create_inputs(wp)
    wp.plots_div = jp.Div(classes='m-2 p-2', a=wp)
    wp.plot_button.on('click', plot_charts)
    return wp
